...
- **SuperJSON 與 contract-event 的關係**:既有 SuperJSON 提案(`idea/v1.2/architecture-improvement/proposal-rpc-superjson.md`,草稿,scope 為 `contract-rpc` + `service-kit/rpc`)明確排除 event payload。本提案的 event serialization 僅在 `service-kit` event codec 內部使用 SuperJSON 作為當前 wire 編碼,屬 event runtime 範疇,與該 RPC proposal 的 scope 不重疊。v1.3 若重寫 SuperJSON proposal,「是否將 SuperJSON 正式納入 event 層」應作為獨立決策審議,不在本提案裁決。
