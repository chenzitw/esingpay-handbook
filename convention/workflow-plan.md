...
寫 plan 的最佳環境是 CLI（直接 codebase 存取）。若不得已在 web 寫，需要先有 codebase survey 文件補足現況查驗（Tim 既有的 `*-codebase-survey-*.md` 就是這個用途）。

## 應涵蓋什麼

- **Context preamble**：簡述驅動本 plan 的 design / blueprint 來源與 scope，讓 plan 對未來讀者自包含。
- **Codebase 現況查驗**：相關既有檔案、pattern、命名慣例的現狀。後續 Steps 才能精確指向。
...
- **測試計劃**：哪些 case 要覆蓋、新增還是補既有測試、unit / integration 邊界。
...
**Closure 自測**：所有 upstream-frozen 的點都被處理？所有開放點都有明確標記（escalation 結論 / known compromise）？
...
| 訊號                                                                  | 屬性         | 處置                  |
| --------------------------------------------------------------------- | ------------ | --------------------- |
| 同一個 design 假設、兩種同樣合理的實作方式，需要選一條                | Plan 層      | plan 內決定、記錄理由 |
